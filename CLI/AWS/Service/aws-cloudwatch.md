# AWS CloudWatch CLI

## Logs
List Log Groups: `aws logs describe-log-groups`
List Log Streams in a Log Group: `aws logs describe-log-streams --log-group-name group1`
List metric streams: `aws cloudwatch list-metric-streams`
Delete a log group: `aws logs delete-log-group --log-group-name group1`
Delete a log stream: 
  `aws logs delete-log-stream --log-group-name group1 --log-stream-name 42831698-e886-4e08-8b10-a8bcc86288d5`
Show logs in a Log Group for the last hour: `aws logs tail --since 1h group1`

### Filters
#### CloudWatch Logs filters
By RequestID: `{ $.requestId = "702fc24f-9eae-4a64-95d1-120ec99e1a3d" }`

#### Logs Insights filters
By RequestID:
```
fields @timestamp, requestId
 | filter requestId = '702fc24f-9eae-4a64-95d1-120ec99e1a3d'
```

## Metrics
List metrics in a Namespace: `aws cloudwatch list-metrics --namespace namespace1`
Put a metric: 
```shell
aws cloudwatch put-metric-data \
  --namespace Namespace1 \
  --metric-data '[
    {
      "MetricName": "Metric1",
      "Value": 123,
      "Unit": "Count"
    }
  ]'
```

## Alarms
List alarms: `aws cloudwatch describe-alarms`
List "in-alarm" alarms: `aws cloudwatch describe-alarms --state-value ALARM`
