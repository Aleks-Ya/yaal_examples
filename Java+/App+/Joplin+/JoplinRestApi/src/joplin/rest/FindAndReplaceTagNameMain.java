...
public class FindAndReplaceTagNameMain {
    public static void main(String[] args) throws IOException {
        ...
        var allowedTagPrefixes = new ArrayList<String>();
        allowedTagPrefixes.add("english");
        allowedTagPrefixes.add("body");
        allowedTagPrefixes.add("finance");
        allowedTagPrefixes.add("fitness");
        allowedTagPrefixes.add("job");
        allowedTagPrefixes.add("moto");
        allowedTagPrefixes.add("oca");
        allowedTagPrefixes.add("place");
        allowedTagPrefixes.add("plan");

        var filteredOriginalTags = originalTags.stream()
                .filter(tag -> allowedTagPrefixes.stream().anyMatch(prefix -> !tag.title().equals(prefix) && tag.title().startsWith(prefix)))
                .toList();
        System.out.println("Filtered original tag number: " + filteredOriginalTags.size());
        System.out.println("Filtered original tags: " + filteredOriginalTags);

        var renamedTags = filteredOriginalTags.stream()
                .map(tag -> new Item(tag.id(), tag.title().replace("_", "/")))
                .toList();
