...
public class SearchableComboBoxApp extends Application {
    ...
    public void start(Stage stage) {
        ...
        searchableComboBox.getSelectionModel().selectedItemProperty().addListener((_, oldValue, newValue) ->
        ...
        searchableComboBox.showingProperty().addListener((_, oldValue, newValue) ->
                out.printf("%d ShowingListener: %s->%s\n", counter.incrementAndGet(), oldValue, newValue));
        searchableComboBox.onShowingProperty().addListener((_, oldValue, newValue) ->
                out.printf("%d OnShowingListener: %s->%s\n", counter.incrementAndGet(), oldValue, newValue));
        searchableComboBox.onShownProperty().addListener((_, oldValue, newValue) ->
