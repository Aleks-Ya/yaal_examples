package lucene10.searching.suggest;
...
class AnalyzingSuggesterTest {
    @Test
    void plainTextDictionary() throws IOException {
        try (var tmpSuggesterDirectory = new ByteBuffersDirectory();
             var analyzer = new StandardAnalyzer()) {

            var dictionaryFile = resourceToPath(getClass(), "AnalyzingSuggesterTest_PlainTextDictionary.txt");
            var dictionary = new PlainTextDictionary(dictionaryFile);

            var tmpFilenamePrefix = "tmp";

            var suggester = new AnalyzingSuggester(tmpSuggesterDirectory, tmpFilenamePrefix, analyzer);
