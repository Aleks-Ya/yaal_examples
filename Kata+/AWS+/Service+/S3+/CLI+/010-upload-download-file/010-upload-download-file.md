...
2. Set env vars
	...
	```
3. Create a bucket: `aws s3 mb s3://$BUCKET`
4. Create a file: `echo abc > /tmp/$ORIGINAL_FILE`
5. Upload the file: 
	1. Upload: `aws s3 cp /tmp/$ORIGINAL_FILE s3://$BUCKET/$ORIGINAL_FILE`
	2. Verify: `aws s3 ls s3://$BUCKET`
6. Download the file: `aws s3 cp s3://$BUCKET/$ORIGINAL_FILE /tmp/$DOWNLOADED_FILE`
7. Compare files: `diff /tmp/$ORIGINAL_FILE /tmp/$DOWNLOADED_FILE`
