# 060-lvm-partitionless-ext4-extend

## Task
Extend the size of a Logical Volume (LVM).

## Setup
1. Set environment variables
```shell
set -x
export DEVICE_FILE=/tmp/lvm-extend.img
export VG_NAME=kata_vg_lvm_extend
export LV_NAME=kata_lv_lvm_extend
export LV_PATH=/dev/$VG_NAME/$LV_NAME
export MOUNT_POINT=/mnt/lvm-extend
export DEVICE_MAPPER_PATH=/dev/mapper/$VG_NAME-$LV_NAME
```
2. Create an LV
	...
	4. Create a LV: `sudo lvcreate -L 50M -n $LV_NAME $VG_NAME`
	5. Use the LV:
...
3. Extend the LV (unmount is NOT needed)
	1. Extend LV:
		1. Show sizes before: `sudo vgdisplay -v $VG_NAME`
		2. Extend LV: `sudo lvextend -L +20M $LV_PATH`
		3. Show sizes after: `sudo vgdisplay -v $VG_NAME`
	2. Extend the file system:
		1. Show size before: `df -h $MOUNT_POINT`
		2. Resize: `sudo resize2fs $DEVICE_MAPPER_PATH`
		3. Show size after: `df -h $MOUNT_POINT`
...
6. Unset env vars: `unset DEVICE_FILE VG_NAME LV_NAME LV_PATH MOUNT_POINT DEVICE_MAPPER_PATH DEVICE_ID`
