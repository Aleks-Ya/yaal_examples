...
def __on_event(webview: AnkiWebView) -> None:
    if webview.kind != AnkiWebViewKind.BROWSER_CARD_INFO:
        return
    showInfo(f"webview_did_inject_style_into_page: {webview}")
...
def __on_event_callback(webview: AnkiWebView) -> None:
    if webview.kind != AnkiWebViewKind.BROWSER_CARD_INFO:
        return
...
if enabled():
    ...
    gui_hooks.webview_did_inject_style_into_page.append(__on_event_callback)
