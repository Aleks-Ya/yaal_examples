...
class SynonymsAntonyms:
    ...
    def _ui_action(self, parent: QWidget):
        op: CollectionOp[OpChanges] = CollectionOp(parent=parent, op=lambda col: self._background_operation(col))
        op.success(self.on_success)
        op.failure(self.on_failure)
        op.run_in_background()

    def menu_action(self, parent: QWidget) -> QAction:
        action = QAction('Fill synonyms and antonyms', parent)
        qconnect(action.triggered, lambda: self._ui_action(parent))
        return action
