...
from app.addon_catalog.common.data_types import AddonInfo, LanguageName
...
class Enricher:
    ...
    def enrich(self, addon_info: AddonInfo) -> AddonInfo:
        ...
        stars: int = self.__github_service.get_stars_count(addon_info.github_repo)
        last_commit: Optional[datetime] = self.__github_service.get_last_commit(addon_info.github_repo)
        action_count: int = self.__github_service.get_action_count(addon_info.github_repo)
        tests_count: int = self.__github_service.get_tests_count(addon_info.github_repo)
        enriched_addon_info: AddonInfo = AddonInfo(addon_info.header, addon_info.github_links,
                                                   addon_info.other_links, addon_info.github_repo,
                                                   languages, stars, last_commit, addon_info.anki_forum_url,
                                                   action_count, tests_count)
    ...
    def __get_languages(self, addon_info: AddonInfo) -> list[LanguageName]:
        if addon_info.github_repo:
            language_dict: dict[LanguageName, int] = self.__github_service.get_languages(addon_info.github_repo)
