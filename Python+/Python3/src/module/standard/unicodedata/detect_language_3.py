...
def __char_to_language(char: str) -> Optional[Language]:
    code_point = ord(char)
...
def detect_language_3(text: str) -> Language:
    ...
    char_language_cache: dict[str, Optional[Language]] = {}
    for char in text:
        if char in char_language_cache:
            language: Language = char_language_cache[char]
        else:
            language: Language = __char_to_language(char)
