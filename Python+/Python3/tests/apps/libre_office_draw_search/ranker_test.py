...
def test_rank():
    search_results: SearchResults = SearchResults([
        SearchResult(0, FodgPath(Path('file1.fodg')), [FileName("file1.fodg")], [PageName("Page 1")], []),
    ...
    assert ranked_search_results == SearchResults([
        SearchResult(1, FodgPath(Path('file2.fodg')), [FileName("file2.fodg")],
                     [PageName("Page 1"), PageName("Page 2")],
                     []),
        SearchResult(2, FodgPath(Path('file1.fodg')), [FileName("file1.fodg")], [PageName("Page 1")], []),
    ], 3, 0, 2)
