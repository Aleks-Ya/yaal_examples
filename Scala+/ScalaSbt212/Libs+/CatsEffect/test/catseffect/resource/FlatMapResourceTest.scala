...
class FlatMapResourceTest extends AsyncFreeSpec with AsyncIOSpec with Matchers {
  "App should" - {

    "flatMap a resource" in {
      ...
      val concat: IO[Unit] =
        (
          for {
            in1 <- fileResource(file1).flatMap(file => Resource.pure(read(file)))
            in2 <- fileResource(file2).flatMap(file => Resource.pure(read(file)))
