package spark3.sql.dataframe.sql

import org.apache.spark.sql.Row
import org.scalatest.flatspec.AnyFlatSpec
import spark3.sql.{Factory, SparkMatchers}

class SqlTest extends AnyFlatSpec with SparkMatchers {

  it should "sum numbers" in {
    val df = Factory.ss.sql("SELECT 1 + 1 AS result")
    df shouldContain """{"result":2}"""
  }
  ...
  it should "use collect_set" in {
    ...
    df shouldContain """{"unique_cities":["Paris","London"]}"""
  }
