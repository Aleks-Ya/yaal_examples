package spark3.sql.dataframe.transformation.column
...
class UpdateColumnTest extends AnyFlatSpec with Matchers {
  ...
  it should "update a column using map transformation" in {
    ...
    val df: DataFrame = Factory.peopleDf.map { row =>
  ...
  it should "rename a nested column" in {
    val df: DataFrame = Factory.createDf("name STRING, details STRUCT<age: INT, gender: STRING>",
