...
class ExistsTest extends AnyFlatSpec with SparkMatchers {

  it should "use exists function" in {
    val df = Factory.createDf("collection STRING, numbers ARRAY<INT>",
      Row("list 1", Seq(1, 4)),
      Row("list 2", Seq(5, 6)),
      Row("list 3", Seq(7, 9)))
    val updatedDf = df.select(col("collection"), exists(col("numbers"), _ % 2 === 0) as "has_even_numbers")
    updatedDf shouldHaveDDL "collection STRING,has_even_numbers BOOLEAN"
  ...
  it should "filter row where a array contains an element" in {
    ...
    updatedDf shouldHaveDDL "collection STRING,numbers ARRAY<INT>"
    updatedDf shouldContain(
  ...
  it should "filter row where a array contains a struct element with non-null field" in {
    val df = Factory.createDf("collection STRING, people ARRAY<STRUCT<name: STRING, age: INT>>",
    ...
    updatedDf shouldHaveDDL "collection STRING,people ARRAY<STRUCT<name: STRING, age: INT>>"
    updatedDf shouldContain(
