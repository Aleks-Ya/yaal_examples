...
class GroupByTransformationTest extends AnyFlatSpec with Matchers {
  ...
  it should "groupBy a string column" in {
    ...
    updatedDf.schema.toDDL shouldEqual "gender STRING,count BIGINT NOT NULL"
  ...
  it should "groupBy with agg" in {
    ...
    updatedDf.schema.toDDL shouldEqual "gender STRING,max_age INT,avg_age DOUBLE,min_age INT"
    updatedDf.toJSON.collect should contain inOrderOnly(
  ...
  it should "groupBy by several columns" in {
    ...
    updatedDf.schema.toDDL shouldEqual "gender STRING,age INT,names ARRAY<STRING> NOT NULL"
    updatedDf.toJSON.collect should contain inOrderOnly(
  ...
  it should "group by an array column" in {
    ...
    unsortedArrayDf.schema.toDDL shouldEqual "countries ARRAY<STRING>,persons ARRAY<STRING> NOT NULL"
    ...
    sortedArrayDf.schema.toDDL shouldEqual "countries ARRAY<STRING>,persons ARRAY<STRING> NOT NULL"
  ...
  it should "groupBy by a struct column" in {
    ...
    updatedDf.schema.toDDL shouldEqual "details STRUCT<age: INT, gender: STRING>,names ARRAY<STRING> NOT NULL"
    updatedDf.toJSON.collect should contain inOrderOnly(
